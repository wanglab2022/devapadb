...
function snp_and_3utr_location(){
	dapars2_res=$1
	extract_SNP_location.py --genotype_bed ./Matrix_eQTL/genotype_matrix.bed --output ${currDir}/Matrix_eQTL/snp_location.txt &
	wait
	extract_3UTR_location.py --dapars_res $dapars2_res --output ${currDir}/Matrix_eQTL/3UTR_location.txt &
	wait
...
function curate_pheno_geno_covariates(){
	...
	known_cov=$2
	topN=$3
	curate_pheno_geno_covariates.R -p $dapars2_res -c $known_cov -n $topN
}
...
function generate_gt_matrix(){
	...
	do
		...
		sample=${filename%.*}
		recode_with_012.py --frq $frq --GT ${currDir}/tmp/${sample}.GT.FORMAT --output ${currDir}/tmp/${sample}.GT.bed &
		wait
	done &
	wait
