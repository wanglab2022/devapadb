...
function generate_configure_for_dapars2(){
	N_cov=$1
	N_threads=$2
	generate_configure_for_Dapars2.py --annotation_3utr ${currDir}/refseq_3utr_annotation.bed \
...
function generate_wigFileList_with_readDepth(){
	...
	extract_read_depth.py --sample_list $bamList --path_wig ${currDir}/wig --output ${currDir}/wigFile_and_readDepth.txt &
	wait
...
function generate_3utr_reference(){
	gene_anno=$1
	refID2Symbol=$2
	DaPars_Extract_Anno.py -b ${gene_anno} -s $refID2Symbol -o ${currDir}/refseq_3utr_annotation.bed &
	wait
